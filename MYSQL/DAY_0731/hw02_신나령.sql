...
/*
1. sqlclass_db 데이터베이스에 제공된 nobel.csv파일을 import하여
각각 nobel 테이블을 생성하고 아래 내용에 대해 쿼리를 작성하시오.
*/
...
select year, subject, winner from nobel
where (subject in ('Physics', 'Peace')) and (year='1960');
...
select * from nobel
where (subject not in ('Chemistry','Medicine')) and (year=1964) order by winner;
